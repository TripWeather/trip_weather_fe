class TripService
  def self.trips_by_uid(uid)
    response = conn.get("/api/v1/#{uid}/trips")
    parse(response)
  end

  def self.trip_by_id(uid, trip_id)
    response = conn.get("/api/v1/#{uid}/trips/#{trip_id}")
    parse(response)
  end

  def self.create_trip(name, uid, departure_date, arrival_date)
    response = conn.post("/api/v1/#{uid}/trips") do |req|
  ...
  def self.delete_trip(uid, trip_id)
    conn.delete("/api/v1/#{uid}/trips/#{trip_id}")
  end

  def self.update_trip(trip_id, name, uid, departure_date, arrival_date)
      response = conn.put("/api/v1/#{uid}/trips/#{trip_id}") do |req|
                  req.params[:trip] = { name: name,
                              uid: uid,
                              departure_date: departure_date,
                              arrival_date: arrival_date }
  ...
  def self.create_stops(trip_id, uid, address, type)
    response = conn.post("/api/v1/#{uid}/trips/#{trip_id}/stops") do |req|
  ...
  def self.all_stops(uid, trip_id)
    response = conn.get("/api/v1/#{uid}/trips/#{trip_id}/stops")
    parse(response)

  end

  def self.address(uid, trip_id, address_id, stop_id)
    response = conn.get("/api/v1/#{uid}/trips/#{trip_id}/stops/#{stop_id}/addresses/#{address_id}")
    parse(response)
  
  end

  def self.update_address(uid, trip_id, address_id, address, stop_id)
    response = conn.put("/api/v1/#{uid}/trips/#{trip_id}/stops/#{stop_id}/addresses/#{address_id}") do |req|
